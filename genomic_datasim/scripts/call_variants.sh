...
do
  ...
  # Call variants #
  #################
  samtools mpileup -t AD,INFO/AD -B -v -f "$ref" readset_sort_nodup.bam | \
    bcftools call -m -f GQ -O z - > variants.vcf.gz
  vcftools --gzvcf variants.vcf.gz --minQ 90 --remove-indels --recode \
    --stdout | gzip - > variants_filtered.vcf.gz
  vcftools --gzvcf variants_filtered.vcf.gz --min-alleles 1 --max-alleles 2 \
    --recode --stdout > variants_biallelic.vcf

  rm variants.vcf.gz
  rm variants_filtered.vcf.gz

  #python3 "$g_dir"/py_scripts/real_vcf.py $g_dir
  #python3 "$g_dir"/py_scripts/vcf_rmv_dup_pos.py variants_biallelic.vcf
  python3 "$g_dir"/py_scripts/vcf_stats.py $g_dir
  gzip variants_biallelic.vcf
  ...
  #samtools depth -a readset_sort_nodup.bam | awk '$3>0 {print $2 " " $3}' > coverages
