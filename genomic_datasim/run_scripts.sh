...
branch_scale=$5
# "ref" or "pop" for reference panel / population phasing
phase_method=$6
...
then
  ...
  if [ $phase_method == "pop" ]
  then
    qsub -o "$g_dir"/logs/std/pre_phase_pop -e "$g_dir"/logs/err/pre_phase_pop \
      "$g_dir"/scripts/pre_phase_pop.sh $g_dir $references

    qsub -t 1-$ancients -o "$g_dir"/logs/std/phase_pop -e "$g_dir"/logs/err/phase_pop \
      "$g_dir"/scripts/phase_pop.sh $g_dir "${#coverages[@]}" "${coverages[@]}" \
      "${#contaminations[@]}" "${contaminations[@]}"
  else
    qsub -t 1-$ancients -o "$g_dir"/logs/std/phase_ref -e "$g_dir"/logs/err/phase_ref \
      "$g_dir"/scripts/phase_ref.sh $g_dir "${#coverages[@]}" "${coverages[@]}" \
      "${#contaminations[@]}" "${contaminations[@]}"
  fi
